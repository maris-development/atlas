...
Python bindings for [ATLAS](../README.md) (Aggregated Tensor Large Array Store), a directory-based store for many similarly-shaped N-dimensional arrays. Install as `pip install atlas-python`; import as `import atlas`.
...
- Local filesystem **and** any [`object_store`](https://docs.rs/object_store) backend
  ...
  `pip install "atlas-python[cloud]"` enables it; without it atlas still works against
...
with atlas.Atlas.create("/tmp/my_store", codec="zstd") as s:   # "zstd" | "lz4" | "none"
...
# Reopen
s2 = atlas.Atlas.open("/tmp/my_store")
...
`xarray` and `dask` ship as required dependencies. Importing `atlas` also registers an xarray accessor at `xr.Dataset.atlas`, so the integration is always available with no extra setup.
...
with atlas.Atlas.create("/tmp/my_store") as atlas:
...
atlas2 = atlas.Atlas.open("/tmp/my_store")
