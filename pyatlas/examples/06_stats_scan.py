...
Run:
    python pyatlas/examples/06_stats_scan.py
"""
import tempfile
...
def main() -> None:
    with tempfile.TemporaryDirectory() as store_dir:
        ...
        with pyatlas.Atlas.create(store_dir, codec="lz4") as atlas:
            for i in range(N_SENSORS):
                ds = atlas.create_dataset(f"sensor_{i:03d}")
                ds.define_array(
                    "readings",
                    dtype="float32",
                    dims=["hour"],
        ...
        print(f"Scanning stats for {len(atlas.list_datasets())} sensors "
              f"(no raw data read)…\n")

        peak_sensor = ""
        peak_max = -np.inf
        for name in sorted(atlas.list_datasets()):
            ds = atlas.open_dataset(name)
        ...
        for name in sorted(atlas.list_datasets())[:5]:
            ds = atlas.open_dataset(name)
            stats = ds.array_stats("readings")
            assert stats is not None
