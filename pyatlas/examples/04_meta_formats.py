...
Run:
    python pyatlas/examples/04_meta_formats.py
"""
import tempfile
...
def populate(atlas: pyatlas.Atlas) -> None:
    ...
    for i in range(30):
        ds = atlas.create_dataset(f"dataset_{i:02d}")
        for j in range(4):
            ds.define_array(
                f"arr_{j}",
                dtype="float32",
...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        atlas = pyatlas.Atlas.open(str(smallest_dir))
        assert len(atlas.list_datasets()) == 30
        print(f"  list_datasets() returned {len(atlas.list_datasets())} datasets ✓")
