...
Run:
    python pyatlas/examples/07_shared_arrays.py
"""
import tempfile
    ...
    with tempfile.TemporaryDirectory() as store_dir:
        ...
        with pyatlas.Atlas.create(store_dir) as atlas:
            for i in range(N_STATIONS):
                ds = atlas.create_dataset(f"station_{i:03d}")
                ds.define_array(
                    "temperature",
                    dtype="float32",
                    dims=["hour"],
            ...
            n_datasets = len(atlas.list_datasets())
            arrays = atlas.list_arrays()
        ...
        t0 = atlas.open_dataset("station_000").read_array("temperature")
        t49 = atlas.open_dataset("station_049").read_array("temperature")
