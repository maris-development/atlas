...
[`pyatlas/benchmarks/`](https://github.com/robinskil/atlas/tree/main/pyatlas/benchmarks).
...
for n in 100 500 1000; do
    python pyatlas/benchmarks/bench_collection.py --case profile --n-vars 4 --atlas-bulk --datasets $n
done

# Gridded, case-default 3 variables.
for n in 100 500 1000; do
    python pyatlas/benchmarks/bench_collection.py --case gridded --atlas-bulk --datasets $n
done
...
for n in 100 500 1000; do
    python pyatlas/benchmarks/bench_collection.py --case profile --n-vars 4 --backends atlas --use-dask --datasets $n
    python pyatlas/benchmarks/bench_collection.py --case gridded --backends atlas --use-dask --datasets $n
done
```
...
```bash
python pyatlas/benchmarks/generate_bench_charts.py    # writes docs/assets/bench_*.svg
```
...
```bash
pip install -e "pyatlas[bench]"      # adds zarr + netCDF4 to your env
...
[`pyatlas/benchmarks/README.md`](https://github.com/robinskil/atlas/blob/main/pyatlas/benchmarks/README.md).
