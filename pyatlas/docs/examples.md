...
[`pyatlas/examples/`](https://github.com/robinskil/atlas/tree/main/pyatlas/examples).
Each one is self-contained, writes to a temp directory, and runs in under
a minute.

| File | What it shows |
|---|---|
| [`01_basics.py`](https://github.com/robinskil/atlas/blob/main/pyatlas/examples/01_basics.py) | Create a store, define arrays, set attributes, reopen, read back. The minimal end-to-end loop. |
| [`02_xarray.py`](https://github.com/robinskil/atlas/blob/main/pyatlas/examples/02_xarray.py) | Round-trip an `xr.Dataset` through atlas using both `atlas.add_xr_dataset(...)` and the `ds.atlas.write(...)` accessor. |
| [`03_dask_streaming.py`](https://github.com/robinskil/atlas/blob/main/pyatlas/examples/03_dask_streaming.py) | Stream a dask-chunked `xr.Dataset` into atlas one chunk at a time, preserving the chunk shape on disk; read back lazily. |
| [`04_meta_formats.py`](https://github.com/robinskil/atlas/blob/main/pyatlas/examples/04_meta_formats.py) | Compare the six metadata format × compression combinations (`atlas.json`, `atlas.msgpack.zst`, …) on a 30-dataset store. Confirms auto-detection on reopen. |
